...
S = G.*exp(-2*G);
  ...
  n=10;
  tams = reshape(tams_quadro,n,n);
  quadros_ent = reshape(quadros_entregues,n,n);
  quadros_trans = reshape(quadros_transmitidos,n,n);
...
mean_result_tq = mean(tams,1);
std_result_tq = std(tams,1);
...
plot(ax1,mean_result_tq,(mean_result_qe./mean_result_qt),'bo',...
  'marker',markers(idx),'linewidth',2);
plot(ax1,mean_result_tq,(mean_result_qe./mean_result_qt),':',...
  'marker',markers(idx),'linewidth',2);
